...
fn main() {
    ...
    println!(
        "HNSW only (bulk)       {:.2} s   ({:.0} vectors/s)",
        hnsw_bulk,
        n as f64 / hnsw_bulk
    );
    ...
    ix2.reserve(n);
    let t = Instant::now();
    for c in items.chunks(2000) {
        ix2.insert_batch(c);
    }
    let hnsw_chunked = t.elapsed().as_secs_f64();
    println!(
        "HNSW only (2000-chunk) {:.2} s ({:.0} vectors/s)",
        hnsw_chunked,
        n as f64 / hnsw_chunked
    );

    println!(
        "\n=> the index is ~{:.0}% of the write time",
        hnsw_chunked / (store_only + hnsw_chunked) * 100.0
    );
    ...
    let img = {
        ...
        d2.execute(&Statement::CreateCollection {
            schema: Schema::new(
                "v",
                vec![Field::new("embed", DataType::Vector(dim, VecPrec::F32))
        ...
        for chunk in vecs.chunks(2000) {
            let docs: Vec<Vec<(String, Expr)>> = chunk
                .iter()
                .map(|v| vec![("embed".to_string(), Expr::Lit(Value::Vector(v.clone())))])
                .collect();
    ...
    while pos < img.len() {
        ...
        let end = p2 + len;
        if rec != 4 {
            nograph.extend_from_slice(&img[start..end]);
        }
        pos = end;
    }
    ...
    println!(
        "graphless image        {:.1} MB",
        nograph.len() as f64 / 1e6
    );
    let t = Instant::now();
    let mut d4 = Database::new();
    d4.load(&nograph).unwrap();
    println!(
        "graphless load         {:.0} ms  (the index is rebuilt)",
        t.elapsed().as_secs_f64() * 1000.0
    );
