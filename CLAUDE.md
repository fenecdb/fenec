...
protocol. `README.md` is the long-form reference (design rationale, benchmarks,
full FenecQL and HTTP surface); this file is the working summary.
...
`json`, `time` (calendar arithmetic), `changes` (the change ring), `plugin`
(registry), `fs` (buffered file I/O, behind the `std-fs` feature).

The browser client is `web/fenec.js` — WASM glue (~165 lines), the query builder,
...
auditable; own codec, own JSON, own HNSW, own SCRAM/crypto. `fenec-core` does
